/***********************************************************************/
/* Author      : Aya Ramzy                                             */
/* Date        : 31 August 2020                                        */
/* Version     : V01                                                   */
/***********************************************************************/

#ifndef HLEDMAT_CONFIG_H
#define HLEDMAT_CONFIG_H


/* Please write pin pair PORT ,PIN*/
#define HLEDMAT_ROW_0      GPIOA,0
#define HLEDMAT_ROW_1      GPIOA,1
#define HLEDMAT_ROW_2      GPIOA,2
#define HLEDMAT_ROW_3      GPIOA,3
#define HLEDMAT_ROW_4      GPIOA,4
#define HLEDMAT_ROW_5      GPIOA,5
#define HLEDMAT_ROW_6      GPIOA,6
#define HLEDMAT_ROW_7      GPIOA,7


#define HLEDMAT_COL_0      GPIOB,0
#define HLEDMAT_COL_1      GPIOB,1
#define HLEDMAT_COL_2      GPIOB,5
#define HLEDMAT_COL_3      GPIOB,6
#define HLEDMAT_COL_4      GPIOB,7
#define HLEDMAT_COL_5      GPIOB,8
#define HLEDMAT_COL_6      GPIOB,9
#define HLEDMAT_COL_7      GPIOB,10

#endif